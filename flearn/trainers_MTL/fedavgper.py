...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        fraction_malicious=self.num_corrupted/len(self.clients)
        benign_testaccuracy=[]
        malicious_testaccuracy=[]
        testaccuracy=[]
        variances=[]
        losses=[]

        maliciousFile = open('malicious.txt', 'r+')
        benignFile = open('benign.txt', 'r+')
        testFile=open('testacc.txt','r+')
        varianceFile=open('variances.txt','r+')
        fractionFile=open('fracion.txt','r+')
        lossesFile=open('losses.txt','r+')
        ...
        for i in range(self.num_rounds + 1):
            if i % self.eval_every == 0 and i > 0:
                ...
                benign_testaccuracy.append(np.sum(num_correct_test[non_corrupt_id]) * 1.0 / np.sum(num_test[non_corrupt_id]))
                malicious_testaccuracy.append(np.sum(num_correct_test[corrupt_id]) * 1.0 / np.sum(num_test[corrupt_id]))
                testaccuracy.append(np.sum(num_correct_test) * 1.0 / np.sum(num_test))
                ...
                losses.append(np.dot(loss_vector, num_train) / np.sum(num_train))
                variances.append(np.var(num_correct_test[non_corrupt_id] / num_test[non_corrupt_id]))
        ...
        print('Saving')
        print("The array is:", malicious_testaccuracy)
        np.savetxt(maliciousFile, np.array(malicious_testaccuracy))
        print("The array is:", benign_testaccuracy)
        np.savetxt(benignFile, np.array(benign_testaccuracy)) 
        print("The array is:", testaccuracy)
        np.savetxt(testFile, np.array(testaccuracy)) 
        print("The array is:", variances)
        np.savetxt(varianceFile, np.array(variances))
        print("The array is:", losses)
        np.savetxt(lossesFile, np.array(losses))
        print("The fraction is:", fraction_malicious)
